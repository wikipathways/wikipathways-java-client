...
public class WikiPathwaysRESTBindingStub implements WikiPathwaysPortType {
	...
	public WSSearchResultLiterature[] findPathwaysByLiterature(String query) throws RemoteException {
		...
		try {
			...
			for (int i = 0; i < pathwayInfoArray.length(); i++) {
				...
				if (matchesQuery(query, id, urlPathway, name, species, revision, authors, description, refs,
						citations)) {
					WSSearchResultLiterature result = new WSSearchResultLiterature(id, urlPathway, name, species,
							revision, authors, description, refs, citations);
	...
	private boolean matchesQuery(String query, String id, String urlPathway, String name, String species,
			String revision, String authors, String description, String refs, String[] citations) {
	...
	public WSOntologyTerm[] getOntologyTermsByPathway(String pwId) throws RemoteException {
		String url = BASE_URL_JSON + "getOntologyTermsByPathway.json";
	...
	public byte[] getPathwayAs(String fileType, String pwId) throws RemoteException {
		...
		String githubUrl = BASE_ASSETS;

		if (!fileType.equals("png") && !fileType.equals("svg") && !fileType.equals("json")
				&& !fileType.equals("gpml")) {
			throw new RemoteException(
					"Unsupported file type: " + fileType + ". Supported types are png, svg, json, gpml.");
		}

		String url = githubUrl + pwId + "/" + pwId + "." + fileType + "?raw=true";
		...
		try {
			...
			try {
				...
				throw new RemoteException("Error while closing input stream for URL " + url + ": " + e.getMessage(),
						e.getCause());
			}
		}
	}
	...
	public WSPathwayInfo[] getPathwaysByOntologyTerm(String term) throws RemoteException {
		String url = BASE_URL_JSON + "getPathwaysByOntologyTerm.json";
	...
	public WSPathwayInfo[] getPathwaysByCommunity(String communityTag) throws RemoteException {
		...
		try {
			...
			throw new RemoteException("Error while processing community tag " + communityTag + ": " + e.getMessage(),
					e);
		}
	}
			...
			for (int i = 0; i < pathwaysArray.length(); i++) {
				...
				for (int j = 0; j < termsArray.length(); j++) {
					...
					if (parent.equalsIgnoreCase(parentTerm)) {
						...
						WSParentOntologyTerm parentOntologyTerm = new WSParentOntologyTerm(wpid, ontology, id, name,
								parent);
	...
	public WSPathwayInfo getPathwayInfo(String pwId) throws RemoteException {
		try {
			...
			throw new RemoteException(
					"Error while processing " + BASE_URL_JSON + "getPathwayInfo.json" + ": " + e.getMessage(),
					e.getCause());
		}
	}
